...
namespace DatabaseLayer.SqlServerProvider.Metadata
{
    internal static class DatabaseIndexProvider
    {
        ...
        internal static DbIndex GetIndex(SqlConnection conn, string name)
        {
            ...
            {
                command.CommandText = "SELECT t.name AS 'TableName', ind.is_unique AS 'IsUnique' FROM sys.indexes ind INNER JOIN sys.tables t ON ind.object_id = t.object_id WHERE ind.name = @IndexName";
                command.Parameters.AddWithValue("@IndexName", name);
