...
namespace DatabaseLayer.SqlServerProvider.Metadata.StoredProcedures
{
    public class DatabaseTable
    {
        ...
        {
            this.ColumnList = new List<DatabaseColumn>();
            using (SqlCommand sqlCommand = new SqlCommand("SELECT \r\n                                ccu.TABLE_NAME,\r\n                                ccu.COLUMN_NAME, \r\n                                ccu.CONSTRAINT_NAME\r\n                            FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS tc\r\n                                INNER JOIN INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE AS ccu  ON tc.CONSTRAINT_NAME = ccu.CONSTRAINT_NAME\r\n                            WHERE \r\n                                tc.TABLE_NAME = @TableName\r\n\t                            AND ccu.CONSTRAINT_NAME LIKE 'PK_%'", conn))
            {
                sqlCommand.Parameters.AddWithValue("@TableName", (object)tableName);
                SqlDataReader sqlDataReader = sqlCommand.ExecuteReader();
                while (((DbDataReader)sqlDataReader).Read())
                    this.PrimaryKey = ((DbDataReader)sqlDataReader)["COLUMN_NAME"].ToString();
                ((DbDataReader)sqlDataReader).Close();
            }
            SqlCommand sqlCommand1 = new SqlCommand("SELECT \r\n                        COLUMN_NAME,\r\n                        IS_NULLABLE,\r\n                        DATA_TYPE,\r\n                        CHARACTER_MAXIMUM_LENGTH,\r\n                        COLUMN_DEFAULT, \r\n                        NUMERIC_PRECISION,\r\n                        NUMERIC_SCALE\r\n                    FROM INFORMATION_SCHEMA.COLUMNS\r\n                    WHERE TABLE_NAME = @TableName", conn);
            sqlCommand1.Parameters.AddWithValue("@TableName", (object)tableName);
            SqlDataReader sqlDataReader1 = sqlCommand1.ExecuteReader();
            while (((DbDataReader)sqlDataReader1).Read())
                this.ColumnList.Add(new DatabaseColumn()
                {
                    ...
                    DefaultValue = string.IsNullOrEmpty(((DbDataReader)sqlDataReader1)["COLUMN_DEFAULT"].ToString()) ? (object)null : ((DbDataReader)sqlDataReader1)["COLUMN_DEFAULT"],
            ...
            ((DbDataReader)sqlDataReader1).Close();
            ...
            sqlCommand2.Parameters.AddWithValue("@TableName", (object)tableName);
            SqlDataReader sqlDataReader2 = sqlCommand2.ExecuteReader();
            while (((DbDataReader)sqlDataReader2).Read())
            {
                string CurrentKey = ((DbDataReader)sqlDataReader2)["CurrentColumn"].ToString();
                DatabaseColumn databaseColumn = this.ColumnList.FirstOrDefault<DatabaseColumn>((Func<DatabaseColumn, bool>)(cl => cl.Name == CurrentKey));
          ...
          ((DbDataReader)sqlDataReader2).Close();
        }
