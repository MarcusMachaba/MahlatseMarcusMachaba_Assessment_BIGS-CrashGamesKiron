            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                try
                {
                    ...
                    using (var command = new SqlCommand("INSERT INTO Logs (Date, Thread, Level, Logger, Message, Exception, MachineName) VALUES (@Date, @Thread, @Level, @Logger, @Message, @Exception, @MachineName)", connection))
                    {
                        ...
                        command.Parameters.Add(new SqlParameter("@Message", SqlDbType.NVarChar, 4000) { Value = loggingEvent.RenderedMessage });
                        command.Parameters.Add(new SqlParameter("@Exception", SqlDbType.NVarChar, 4000) { Value = loggingEvent.GetExceptionString() });
